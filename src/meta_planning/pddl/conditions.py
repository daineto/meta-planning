...
class Conjunction(JunctorCondition):
    ...
    def __str__(self):
        return "(and %s)" % (' '.join(map(str, self.parts)))
...
class Fluent(Condition):
    ...
    def __repr__(self):
        return "Fluent(predicate: % r, args: % r)" % (self.predicate, self.args)
...
class Literal(Fluent):
    __hash__ = Condition.__hash__
    def __init__(self, predicate, args, valuation):
        Fluent.__init__(self, predicate, args)
    ...
    def __repr__(self):
        return "Literal(predicate: % r, args: % r, valuation: %r)" % (self.predicate, self.args, self.valuation)
    ...
    def rename_variables(self, renamings):
        new_args = tuple(renamings.get(arg, arg) for arg in self.args)
        return Literal(self.predicate, new_args, self.valuation)
